...
function readJsonStreaming(filePath) {
...
async function writeValue(stream, value) {
    if (value === null || value === undefined) {
        ...
        for (let i = 0; i < value.length; i++) {
            ...
            await writeValue(stream, value[i]);
        }
        await writeToStream(stream, ']');
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            await writeValue(stream, value[keys[i]]);
        }
        await writeToStream(stream, '}');
    } else {
...
function writeJsonStreaming(filePath, obj) {
    return new Promise((resolve, reject) => {
        const stream = fs.createWriteStream(filePath);
        stream.on('error', reject);
        stream.on('finish', resolve);
        writeValue(stream, obj).then(() => stream.end()).catch(reject);
    });
}
...
async function updateChainSpec(inputFile, outputFile) {
    ...
    let chainSpec;
    try {
        chainSpec = await readJsonStreaming(inputFile);
    } catch (err) {
    ...
    // Save the updated chain spec using streaming writer
    try {
        await writeJsonStreaming(outputFile, chainSpec);
